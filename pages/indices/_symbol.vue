...
export default {
    ...
    methods: {
      ...
      fetchPrice() {
        ...
        this.$axios.$get(`https://api.finage.co.uk/last/index/${i.symbol}?apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
          console.log("Price")
      ...
      fetchAggregates(){
        let i = this.indices.find( item => item.name === this.symbol.replace('-', ' ').toUpperCase());
        this.$axios.$get(`https://api.finage.co.uk/agg/index/${i.symbol}/1day/2021-01-01/${this.yesterday}?limit=1825&&apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
