...
export default {
    ...
    methods: {
      fetchDetails() {
        ...
        this.$axios.$get(` https://api.finage.co.uk/agg/index/${itemSymbol}/1day/2021-02-05/2021-03-02?apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
      ...
      fetchPrice() {
        ...
        this.$axios.$get(`https://api.finage.co.uk/bonds/us/rate/${i.symbol}?apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
          console.log("Price")
      ...
      fetchAggregates(){
        let i = this.bonds.find(item => item.name.toLowerCase() === this.symbol.replace(/-/g, ' '));
        this.$axios.$get(`https://api.finage.co.uk/agg/index/${i.symbol}/1/day/2021-01-01/${this.yesterday}?limit=1825&apikey=${process.env.FINAGE_API_KEY}`)
          .then(response => {
      ...
      fetchNews(){
        let i = this.bonds.find(item => item.name.toLowerCase() === this.symbol.replace(/-/g, ' '));
        this.$axios.$get(`https://api.finage.co.uk/news/forex/${i.symbol}?apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
