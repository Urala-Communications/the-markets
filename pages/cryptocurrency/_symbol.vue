...
export default {
    ...
    methods: {
        fetchDetails() {
            ...
            this.$axios
                .$get(
                    `https://api.finage.co.uk/detail/cryptocurrency/${this.symbol}?apikey=${process.env.FINAGE_API_KEY}`
                )
                .then((response) => {
                    // console.log("Details")
        ...
        fetchPrice() {
            ...
            this.$axios
                .$get(
                    `https://api.finage.co.uk/last/crypto/detailed/${i.symbol}?apikey=${process.env.FINAGE_API_KEY}`
                )
                .then((response) => {
        ...
        checkMarketStatus() {
            this.$axios
                .$get(
                    `https://api.finage.co.uk/marketstatus?apikey=${process.env.FINAGE_API_KEY}`
                )
                .then((response) => {
        ...
        fetchAggregates() {
            ...
            this.$axios
                .$get(
                    `https://api.finage.co.uk/agg/crypto/${i.symbol}/1/day/${fiveYearsAgo}/${this.today}?limit=5000&apikey=${process.env.FINAGE_API_KEY}`
                )
                .then((response) => {
        ...
        startUpdateData(symbol, range, limit, interval, minDate, startPoint) {
            ...
            this.$axios
                .$get(
                    `https://api.finage.co.uk/agg/crypto/${symbol}/1/${interval}/${minDate}/${this.today}?&apikey=${process.env.FINAGE_API_KEY}&limit=${limit}`
                )
                .then((response) => {
                    console.log("Aggregates");
                    //console.log(response.results);
        ...
        fetchNews() {
            ...
            this.$axios
                .$get(
                    `https://api.finage.co.uk/news/cryptocurrency/${i.icon}?apikey=${process.env.FINAGE_API_KEY}`
                )
                .then((response) => {
