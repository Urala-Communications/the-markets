...
export default {
    ...
    methods: {
      fetchDetails() {
        ...
        this.$axios.$get(`https://api.finage.co.uk/agg/forex/prev-close/${i.symbol}?apikey=${process.env.FINAGE_API_KEY}`)
          .then(response => {
            console.log("Details")
      ...
      fetchPrice() {
        ...
        let i = this.commodities.find( item => item.icon.toLowerCase() === this.symbol);
        this.$axios.$get(`https://api.finage.co.uk/last/trade/forex/${i.symbol}?apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
          console.log("Price")
      ...
      fetchAggregates(){
        let i = this.commodities.find( item => item.icon.toLowerCase() === this.symbol);
        this.$axios.$get(`https://api.finage.co.uk/agg/forex/${i.symbol}/1/day/2021-01-01/${this.yesterday}?limit=1825&apikey=${process.env.FINAGE_API_KEY}`)
        .then(response => {
