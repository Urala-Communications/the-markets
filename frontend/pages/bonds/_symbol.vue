...
export default {
  ...
  head() {
    return {
      title:
        this.symbol.replace(/-/g, " ").toUpperCase() +
        " $" +
        this.item.price +
        " - " +
        "The Markets",
  ...
  methods: {
    async fetchDetails() {
      ...
      ["open", "close", "low", "high", "volume"].forEach((field) => {
        this[field] = res.results[0][field.charAt(0)];
      });
    },
    async fetchPrice() {
      ...
      //   });
      // };
      let i = this.bonds.find(
        (item) => item.name.toLowerCase() === this.symbol.replace(/-/g, " ")
      );
      this.$set(this.item, "name", i.name);

      const res = await useQuery({
        query: "finage.bonds",
        variables: { suffix: "us/rate", symbol: i.symbol },
        axios: this.$axios,
      });

      if (!res) return;

      this.item.price = res.value.toFixed(2);
      this.$set(this.item, "icon", i.icon);
      this.loading = false;
    },
    async checkMarketStatus() {
      const res = await methods.fetchMarketStatus(this.$axios);
      if (res?.nasdaq) this.marketStatus = res?.nasdaq;
    },
    async fetchAggregates() {
      ...
      if (res?.results?.length) {
        ...
        this.$axios
          .$get(
            `https://api.tradingeconomics.com/markets/historical/${i.symbol}?c=${this.tradingEconKey}&f=json`
          )
          .then((response) => {
            if (response.length) {
    ...
    async updateInterval(symbol, interval, text) {
      if (symbol === this.live) {
        ...
        const [period, multiple = ""] = text.split("/");
        const res = await useQuery({
          query: "finage.agg",
          variables: {
            suffix: "index",
    ...
    fetchNews() {
      ...
      this.$axios
        .$get(
          `https://api.finage.co.uk/news/forex/${i.symbol}?apikey=${this.finageApiKey}`
        )
        .then((response) => {
          let filteredNews = response.news.filter(
