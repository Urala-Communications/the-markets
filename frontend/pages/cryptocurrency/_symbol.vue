  ...
  methods: {
    ...
    checkBackground() {
      ...
      document.querySelectorAll(".icon").forEach(function (icon) {
        if (
          window.getComputedStyle(icon).backgroundImage == "none" &&
          self.item.logo
        ) {
    ...
    async fetchAggregates() {
      ...
      if (!binanceCheck) {
        let finageCheck = await this.fetchDataByFinage(
          this.item.symbol.toLowerCase() + "usd",
          "1d"
        );
        if (!finageCheck) {
          let coinGeckoCheck = await this.fetchDataByCoinGecko(
            this.item.symbol,
            "1"
          );
    ...
    fetchAllResursive(symbol, interval, lastdate) {
      if (this.stopRun) {
        ...
        this.$axios
          .$get(
            `https://api.finage.co.uk/agg/crypto/${symbol.slice(
              0,
              -1
            )}/1/week/${last}/${lastdate}?apikey=${this.finageApiKey}`
          )
          .then((response) => {
            if (response.results) {
          ...
          this.$axios
            .$get(
              `https://api.finage.co.uk/agg/crypto/${symbol.slice(
                0,
                -1
              )}/1/week/${this.fiveYearsAgo}/${this.today}?apikey=${
                this.finageApiKey
              }`
            )
            .then((response) => {
    ...
    async fetchDataByFinage(symbol, interval) {
      ...
      try {
        let res = await this.$axios.$get(
          `https://api.finage.co.uk/agg/crypto/${symbol}/${text}/${last}/${this.today}?apikey=${this.finageApiKey}`
        );
      ...
      try {
        let res = await this.$axios.$get(
          `https://api.coingecko.com/api/v3/coins/${
            this.item.slug
              ? this.item.slug.toLowerCase()
              : this.item.symbol.toLowerCase()
          }/ohlc?vs_currency=usd&days=${day}`
        );
        // console.log("cg res", res);
        if (res) {
          this.chartData = res.map((o) => {
            const [timestamp, openPrice, high, low, close, volume] = [...o, 0];
            return [timestamp, openPrice, high, low, close, volume].map((n) =>
              Number(n)
            );
          });
