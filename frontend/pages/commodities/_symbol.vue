...
export default {
  ...
  methods: {
    async fetchDetails() {
      ...
      this.$axios
        .$get(
          `https://api.finage.co.uk/agg/forex/prev-close/${symbol}?apikey=${this.finageApiKey}`
        )
        .then((response) => {
    ...
    fetchPrice() {
      ...
      this.$axios
        .$get(
          `https://api.finage.co.uk/last/trade/forex/${symbol}?apikey=${this.finageApiKey}`
        )
        .then((response) => {
          this.$set(this.item, "icon", i.icon);
    ...
    async updateInterval(symbol, interval, text) {
      if (symbol === this.live) {
        ...
        const [period, multiplier] = text.split("/");
  ...
  created() {
    this.$root.$on("updateCommodity", (item) => {
      if (item.name.replace(/ /g, "-").toLowerCase() === this.symbol) {
        let i = this.commodities.findIndex(
