      ...
      <div class="row m-0 index-list" id="indices">
        ...
        <div class="col-lg-8 offset-lg-4">
          <div class="col-12 white-well pt-2">
            <IndexList
              :data="indices.filter((item) => item.type === 'indice')"
              indexPage
              type="indices"
            />
...
export default {
  ...
  methods: {
    fetchIndice(symbol) {
      useQuery({
        ...
        .then((response) => {
          let indexFound = this.indices.findIndex(
            (indice) => indice.symbol === response.symbol
          );
  ...
  created() {
    this.filteredIndices = this.indices.filter(
      (item) => item.type === "indice"
    );
